...
class MHSAttention(nn.Module):
    """Applies multihead self-attention (MHSA) mechanism on the `context` using the `query`.
    Args:
        dimensions (int): Dimensionality of the query and context.
    ...
    def __init__(self, dimensions, num_heads= 1):
        ...
        self.mhsa = nn.MultiheadAttention(dimensions, num_heads, 
                                          batch_first= True)
    ...
    def forward(self, query, context, attention_mask):
        ...
        Args:
            query (:class:`torch.FloatTensor` [batch size, output length, dimensions]): Sequence of
                queries to query the context (i.e. Bert embedding)
            context (:class:`torch.FloatTensor` [batch size, query length, dimensions]): Data
                overwhich to apply the attention mechanism. (i.e. Intent embedding)
            output length: length of utterance
            query length: length of each token (1)
        ...
        # mix.shape = (batch_size, output_len, hidden_size)
        if attention_mask is not None:
            mix, attention_weights = self.mhsa(query, context, context, 
...
class SlotClassifier(nn.Module):
    def __init__(
        ...
        if self.use_intent_context_concat:
            ...
            output_dim = self.attention_embedding_size
            if self.attention_mechanism == 'align':
                self.attention = Attention(attention_embedding_size)
            elif self.attention_mechanism == 'mhsa':
                self.attention = MHSAttention(attention_embedding_size, num_heads)
