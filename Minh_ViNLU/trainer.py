...
class Trainer(object):
    ...
    def train(self):
        ...
        if self.args.optimizer == 'adamw':
            ...
            raise NotImplementedError('Optimzer not found!')
        
        if self.args.scheduler == 'linearwa':
            scheduler = get_linear_schedule_with_warmup(
                optimizer, num_warmup_steps=self.args.warmup_steps, num_training_steps=t_total
            )
        elif self.args.scheduler == 'cosine':
            scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max= t_total)
        elif self.args.scheduler == 'cosinewa':
                ...
                if (step + 1) % self.args.gradient_accumulation_steps == 0:
                    ...
                    if self.args.logging_steps > 0 and global_step % self.args.logging_steps == 0:
                        ...
                        if self.args.verbose:
                            print("***** Evaluations *****")
                            print("Loss/validation", results["loss"])
                            print("Intent Accuracy/validation", results["intent_acc"])
                            print("Slot F1/validation", results["slot_f1"])
                            print("Mean Intent Slot", results["mean_intent_slot"])
                            print("Sentence Accuracy/validation", results["semantic_frame_acc"])
                            print("***** ########### *****")
